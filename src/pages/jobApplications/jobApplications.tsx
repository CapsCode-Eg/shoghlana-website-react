                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-5">
                        {
                            data?.applications?.map((application: any) => (
                                <div key={application.id} className="p-4 bg-white shadow-lg rounded-lg relative">
                                    <div className="flex flex-row items-center gap-4">
                                        {
                                            application?.user_id?.image ?
                                                <img src={application?.user_id?.avatar} alt="Avatar" className="w-12 h-12 rounded-full" />
                                                :
                                        ...
                                        <div className="flex flex-col ">
                                            <div className="flex flex-row items-center gap-0.5">
                                                <span className="font-medium text-[18px] text-black">{application?.user_id?.first_name.charAt(0).toUpperCase() + application?.user_id?.first_name.slice(1)} {application?.user_id?.last_name.charAt(0).toUpperCase() + application?.user_id?.last_name.slice(1)}</span>
                                                <span className="text-[14px] text-gray-500">({application?.user_id?.seeker?.job_title.charAt(0).toUpperCase() + application?.user_id?.seeker?.job_title.slice(1)})</span>
                                            </div>
                                    ...
                                    <div className="flex flex-row flex-wrap items-center mt-1 ms-1">
                                        <div className="flex flex-row items-center ">
                                            <span className="text-sm text-gray-500">{application?.user_id?.education?.university.charAt(0).toUpperCase() + application?.user_id?.education?.university.slice(1)}</span>
                                        ...
                                        <div className="flex flex-row items-center  ">
                                            <span className="text-sm text-gray-500">{application?.user_id?.years_of_experience || 0}</span>
                                            <span className="mx-1">-</span>
                                    ...
                                    </div>
                                    {
                                        application?.status === 'accepted' ?
                                            <span className="absolute top-4 end-4 text-[13px] text-[#0055D9]">Accepted</span>
                                            :
                                            <Link
                                                to={`/job_applications/details/${application.id}`}
                                                className="flex flex-row items-center justify-center gap-1 px-[6px] border border-[#0055D9] w-fit h-[26px] rounded-[5px] absolute top-4 end-4 hover:cursor-pointer"
                                            >
                                                ...
                                                <span className="text-[#0055D9] text-[13px] font-semibold">View Application</span>
                                            </Link>
                                    }
                                </div>
                            ))
